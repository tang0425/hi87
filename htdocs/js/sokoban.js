 ...
 *  @author      tang0425(powerlife0983@gmail.com)
 *  @date       11/17/2017 created.
 *  @date       01/05/2018 last modified.
 *  @version    0.1.0
 *  @since      0.1.0
 *  @copyright  MIT, © 2017-2018 tang0425
 *  @details
 *
  ...
  update: function (e) {
    ...
    for(var y=0; y<this.level.length; y++){
      for(var x=0; x<this.level.length; x++){
        if(this.level[y].charAt(x)== SOKOBAN.BOX_ON_GOAL ) boxyo=1;
        if(this.level[y].charAt(x)== SOKOBAN.GOAL ||
            this.level[y].charAt(x)== SOKOBAN.MAN_ON_GOAL ) noyo=0;
      }
    }
